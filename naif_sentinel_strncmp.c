...
int naive_research_sentinel(char *text, char *word){
    ...
    while (1)
    {
        ...
        if (text_sentinel[i]==word[0])
        {
            ...
            if (strncmp(text_sentinel + i+1, word+1 , word_len-1 )==0){
                /*
...
int main() {
    ...
    for (int i = 0; i < num_textes; i++) {
        ...
        for (int j = 0; j < num_mots; j++) {
            ...
            enregistrer_resultats(fichier_csv, textes[i], mots[j], "Naive strncmp  avec boucle rapide avec sentinelle", temps_execution);
