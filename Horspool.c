...
int HORSPOOL(char *x, char *y) {
   ...
   //remplissage de la tab dern occ
...
int main() {
    ...
    for (int i = 0; i < num_textes; i++) {
        ...
        for (int j = 0; j < num_mots; j++) {
            ...
            double temps_execution = mesurer_temps(HORSPOOL, text, mot);
            
            // Calculer le nombre d'occurrences trouvé
            int occurrences = HORSPOOL(text, mot);
            ...
            enregistrer_resultats(fichier_csv, textes[i], mots[j], "HORSPOOL", temps_execution);
