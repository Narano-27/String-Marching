...
// Fonction de recherche naïve qui retourne le nombre d'occurrences trouvées
int naive_research(char *text, char *word) {
...
int main() {
    ...
    for (int i = 0; i < num_textes; i++) {
        ...
        for (int j = 0; j < num_mots; j++) {
            ...
            double temps_execution = mesurer_temps(naive_research, text, mot);
            
            // Calculer le nombre d'occurrences trouvé
            int occurrences = naive_research(text, mot);
            ...
            enregistrer_resultats(fichier_csv, textes[i], mots[j], "Naive", temps_execution);
