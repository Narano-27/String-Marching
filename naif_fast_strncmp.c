...
// avec boucle rapide sans sentinelle
int naive_fast_research(char *text, char *word){
    ...
    for (int i=0 ; i<text_len-word_len+1;i++){
        ...
        if (text[i]==word[0])
        {
            if(strncmp(text + i+1, word+1 , word_len-1 )==0){
            cpt++;
            }
        }              
    }
...
int main() {
    ...
    for (int i = 0; i < num_textes; i++) {
        ...
        for (int j = 0; j < num_mots; j++) {
            ...
            double temps_execution = mesurer_temps(naive_fast_research, text, mot);
            
            // Calculer le nombre d'occurrences trouvé
            int occurrences = naive_fast_research(text, mot);
            ...
            enregistrer_resultats(fichier_csv, textes[i], mots[j], "Naive strncmp  avec boucle rapide sans sentinelle", temps_execution);
